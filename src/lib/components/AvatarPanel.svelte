<script lang="ts">
  ...
  function syncMouthAlignToPopup() {
    try {
      const raw = localStorage.getItem('cohost_mouth_align_v1');
      if (!raw || !avatarChannel) return;
      const align = JSON.parse(raw);
      avatarChannel.postMessage({ type: 'set_mouth_align', align });
    } catch {
      // no-op
    }
  }
  ...
  async function loadSavedAvatar() {
    try {
      ...
      selectedName = saved.fileName || 'saved-avatar';
  ...
  async function openPopup() {
    ...
    try {
      ...
      if (existing) {
        ...
        window.setTimeout(() => syncMouthAlignToPopup(), 120);
        return;
      }
      ...
      window.setTimeout(() => syncMouthAlignToPopup(), 260);
    } catch (error) {
...
<section class="card grid">
  ...
  <small class="muted">Pick one photo, then open popup. Bot responses animate speaking state in real-time.</small>
  {#if activationBlocked}
    <small class="muted">{activationBlockedReason}</small>
  {/if}
  <input type="file" accept="image/*" on:change={onPhotoPick} disabled={activationBlocked} />
  {#if selectedName}<small>Saved avatar: {selectedName}</small>{/if}
  <button on:click={openPopup} disabled={!ready || activationBlocked}>Open Avatar Popup</button>
